// ++++++++++++++++ Define Contracts ++++++++++++++++ 
const {FUNDATION_ADDRESS, BURN_ADDRESS, GOF_STRATEGY_CAKE_LP } = require('./config');
const knownContracts = require('./known-contracts');
const {writeLog} = require('./log');

const Controller = artifacts.require("GOFControllerV1");
const strategyForCakeLp = artifacts.require("StrategyForCakeLp");
// ++++++++++++++++  Main Migration ++++++++++++++++ 
...
async function deploy(deployer, network) {
    ...
    for await (const { pid, token, output, liquidityAToken, liquidityBToken, pool, router, hgofRouting, swap2LiquidityARouting, swap2LiquidityBRouting } of GOF_STRATEGY_CAKE_LP) {
        ...
        await deployer.deploy(strategyForCakeLp, controller.address, pid, tokenAddress, outputAddress, liquidityATokenAddress, liquidityBTokenAddress, pool, router, BURN_ADDRESS);
        console.log(`[GOF] Deploy GofStrategy[${token}] = ${strategyForCakeLp.address}`);
        deployments[token] = strategyForCakeLp.address;
        ...
        console.log(`[GOF] set Routing swap2TokenARouting ${swap2TokenARouting}`);

        for (var routing of swap2LiquidityBRouting) {
          swap2TokenBRouting.push(knownContracts[routing][network]);
        }

        console.log(`[GOF] set Routing swap2TokenBRouting ${swap2TokenBRouting}`);

        const strategyImpl = await strategyForCakeLp.deployed();
    ...
    await writeLog(deployments, 'strategy_cake_lp', network);
}